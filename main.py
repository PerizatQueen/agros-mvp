...
def dashboard():
    ...
    # Цель дохода по договору + прогресс к ней (концепция: одна цель всегда «мигает»)
...
def _task_steps(title, plot, lg):
    t = (title or '').lower()
    p = plot or ('участок' if lg == 'ru' else 'учаске')
    ru = {
        'полив': ['Включите капельный полив на участке на 40 минут.', 'Проверьте, что вода доходит до крайних рядов.', 'Сфотографируйте участок во время полива.'],
        'обрезк': ['Уберите сухие и повреждённые ветви.', 'Сформируйте крону по технологии.', 'Сфотографируйте результат обрезки.'],
        'вредител': ['Приготовьте раствор по инструкции.', 'Равномерно опрыскайте деревья.', 'Сфотографируйте обработанный участок.'],
        'удобрен': ['Внесите удобрение под корень по норме.', 'Полейте после внесения.', 'Сфотографируйте процесс внесения.'],
        'болезн': ['Осмотрите листья на признаки болезней.', 'Отметьте проблемные деревья.', 'Сфотографируйте поражённые листья.'],
        'сбор': ['Проверьте зрелость плодов.', 'Соберите пробную партию.', 'Сфотографируйте собранный урожай.'],
    }
    kz = {
        'полив': ['Учаскеде тамшылатып суаруды 40 минутқа қосыңыз.', 'Су шеткі қатарларға жететінін тексеріңіз.', 'Суару кезінде учаскені суретке түсіріңіз.'],
        'обрезк': ['Құрғақ және зақымдалған бұтақтарды алыңыз.', 'Тәжді технология бойынша қалыптастырыңыз.', 'Нәтижені суретке түсіріңіз.'],
        'вредител': ['Ерітіндіні нұсқау бойынша дайындаңыз.', 'Ағаштарды біркелкі бүркіңіз.', 'Өңделген учаскені суретке түсіріңіз.'],
        'удобрен': ['Тыңайтқышты норма бойынша тамырға енгізіңіз.', 'Енгізгеннен кейін суарыңыз.', 'Процесті суретке түсіріңіз.'],
        'болезн': ['Жапырақтарды ауру белгілеріне тексеріңіз.', 'Проблемалы ағаштарды белгілеңіз.', 'Зақымдалған жапырақтарды түсіріңіз.'],
        'сбор': ['Жемістердің пісуін тексеріңіз.', 'Сынама партиясын жинаңыз.', 'Жиналған өнімді суретке түсіріңіз.'],
    }
    table = kz if lg == 'kz' else ru
    for k, v in table.items():
        if k in t:
            return v
    if lg == 'kz':
        return ['Тапсырманы технология бойынша орындаңыз.', 'Нәтижені тексеріңіз.', 'Учаскені суретке түсіріңіз.']
    return ['Выполните работу по технологии.', 'Проверьте результат.', 'Сфотографируйте участок.']
...
def task_detail(task_id):
    uid = session['user_id']
    rows = db.db_get('tasks', {'id': f'eq.{task_id}', 'user_id': f'eq.{uid}'}) or []
    if not rows:
        return redirect(url_for('tasks'))
    t = rows[0]
    plot_name = ''
    if t.get('plot_id'):
        pr = db.db_get('plots', {'id': f'eq.{t["plot_id"]}'}) or []
        plot_name = pr[0].get('name', '') if pr else ''
    t['plot_name'] = plot_name
    steps = _task_steps(t.get('title', ''), plot_name, lang())
    return render_template('task_detail.html', task=t, steps=steps, lang=lang())
...
def bonus_shop():
    ...
    try:
        ...
        db_items = []
    if db_items:
        _icons = {'кофе': '☕', 'удобрен': '🧴', 'тыңайт': '🧴', 'ящик': '🍎', 'фрукт': '🍎', 'жеміс': '🍎',
                  'инсектицид': '🧪', 'ножниц': '✂️', 'қайшы': '✂️', 'перчат': '🧤', 'қолғап': '🧤', 'консультац': '📊', 'кеңес': '📊', 'саженц': '🌱', 'көшет': '🌱'}
        ...
        def _picon(nm):
            ...
            return '🎁'
        shop_items = [{
            'id': it['id'], 'icon': _picon(it.get('name', '')), 'name': it.get('name', ''),
            'description': it.get('description') or ('В пункте приёмки' if lg == 'ru' else 'Қабылдау пунктінде'),
            'price': it.get('cost', 0) or 0,
        } for it in db_items]
    else:
        shop_items = [
...
def chat_message():
    ...
    try:
        ...
        lg = lang()
        if lg == 'kz':
            sys_prompt = ('Сен — AgrOS платформасының AI-агрономысың. Қазақстан фермерлеріне көмектесесің. '
                          'Қысқа жауап бер (3-4 сөйлем), сәлемдесусіз, бірден практикалық кеңес айт. '
                          'ТЕК ҚАЗАҚ ТІЛІНДЕ жауап бер. Орыс тілін мүлдем қолданба, тілдерді араластырма.')
        else:
            sys_prompt = ('Ты — AI-агроном платформы AgrOS. Помогаешь фермерам Казахстана. '
                          'Отвечай коротко (3-4 предложения), без приветствий, сразу давай практический совет. '
                          'Отвечай ТОЛЬКО на русском языке. Не используй казахский, не смешивай языки.')
        _resp = _req.post(
